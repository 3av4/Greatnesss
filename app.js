// تطبيق مجموعة العظمة - النسخة المحسنة المعدلة
// إصدار 5.0 - مع جميع التصحيحات المطلوبة
...
const appData = {
    ...
    chatRewards: {
        currentStage: 1,
        messagesInCurrentStage: 0,
        stages: [
            { messages: 10, reward: 20 },
            { messages: 20, reward: 30 },
            { messages: 30, reward: 40 },
            { messages: 40, reward: 50 },
            { messages: 50, reward: 60 }
        ],
        lastReset: Date.now(),
        resetInterval: 3600000 // ساعة واحدة
    },
    // نظام منع السبام
    spamDetection: {
        lastMessages: [],
        spamThreshold: 3,
        muteDuration: 300000 // 5 دقائق
    }
};
...
const shopItems = {
    ...
    "name-colors": [
        { 
            ...
            type: "name-color", 
            color: "#FFD700",
            description: "لون ذهبي مشع ومبهر يظهر للجميع في الشات",
            cssClass: "name-color-gold"
        },
        { 
            id: "name-color-silver", 
            name: "لون فضي مشع", 
            price: 12000, 
            type: "name-color", 
            color: "#C0C0C0",
            description: "لون فضي مشع وأنيق يلفت الأنظار",
            cssClass: "name-color-silver"
        },
        { 
            id: "name-color-blue", 
            name: "لون أزرق مشع", 
            ...
            type: "name-color", 
            color: "#1E90FF",
            description: "لون أزرق مشع بتدرجات جذابة",
            cssClass: "name-color-blue"
        },
        { 
            id: "name-color-purple", 
            name: "لون بنفسجي مشع", 
            price: 18000, 
            type: "name-color", 
            color: "#8A2BE2",
            description: "لون بنفسجي مشع وساحر",
            cssClass: "name-color-purple"
        }
    ],
    backgrounds: [
        { 
    ...
    logos: [
        ...
        { id: "🍰", name: "كيك", price: 9999, type: "logo", description: "بنتي" },
        { id: "🐢", name: "الشعار", price: 800, type: "logo", description: "لو تسرب بنتناك" },
        { id: "😳", name: "شوووووو", price: 1500, type: "logo", description: "مصدوم" },
        { id: "♟", name: "شعار شطرنج", price: 900, type: "logo", description: "او يس شطرنج" },
        { id: "🏹", name: "ملك القوس", price: 880, type: "logo", description: "قناص" },
        { id: "😎", name: "شعار للقوي", price: 3300, type: "logo", description: "قوة" },
    ]
};
...
const elements = {
    ...
    // نظام المكافآت
    chatProgressFill: document.getElementById('chatProgressFill'),
    currentMessages: document.getElementById('currentMessages'),
    targetMessages: document.getElementById('targetMessages'),
    ...
    // نموذج الميوت
    muteReasonSelect: document.getElementById('muteReasonSelect'),
    customMuteReason: document.getElementById('customMuteReason'),
...
// تهيئة التطبيق
function initApp() {
    console.log('🚀 بدء تشغيل تطبيق مجموعة العظمة - النسخة المحسنة المعدلة...');
    
    // إزالة أي تخزين محلي سابق (إزالة نظام تذكرني)
...
function loadFirebaseData() {
    ...
    // تحميل سجل التحويلات
    database.ref('transfers').limitToLast(50).on('value', (snapshot) => {
        if (snapshot.exists()) {
            const transfers = [];
            const data = snapshot.val();
            for (const id in data) {
            ...
            updateTransferHistory(transfers);
        }
    });
    
    // تحميل نظام المكافآت
    loadChatRewardsFromFirebase();
}

// تحميل نظام المكافآت من Firebase
function loadChatRewardsFromFirebase() {
    if (!appData.currentUser) return;
    
    database.ref(`chatRewards/${appData.currentUser.id}`).once('value')
        .then(snapshot => {
            if (snapshot.exists()) {
...
function updateRewardsDisplay() {
    if (!elements.chatProgressFill || !elements.currentMessages || 
        !elements.targetMessages || !elements.currentReward) return;
    
    const currentStage = appData.chatRewards.currentStage;
    const stage = appData.chatRewards.stages[currentStage - 1];
    const messages = appData.chatRewards.messagesInCurrentStage;
    
    // تحديث شريط التقدم
    const percentage = (messages / stage.messages) * 100;
    elements.chatProgressFill.style.width = Math.min(percentage, 100) + '%';
    
    // تحديث الأرقام
    elements.currentMessages.textContent = messages;
    elements.targetMessages.textContent = stage.messages;
    elements.currentReward.textContent = stage.reward + ' نقطة';
...
// تحديث مؤقت إعادة التعيين
function updateResetTimer() {
    if (!elements.resetTimer) return;
    
    const now = Date.now();
    const elapsed = now - appData.chatRewards.lastReset;
    const remaining = Math.max(0, appData.chatRewards.resetInterval - elapsed);
    
    const minutes = Math.floor(remaining / 60000);
    const seconds = Math.floor((remaining % 60000) / 1000);
    
    elements.resetTimer.textContent = `${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`;
}

// التحقق من إعادة تعيين النظام
function checkRewardsReset() {
    const now = Date.now();
    if (now - appData.chatRewards.lastReset >= appData.chatRewards.resetInterval) {
        // إعادة تعيين النظام
        appData.chatRewards = {
            currentStage: 1,
            messagesInCurrentStage: 0,
        ...
        saveChatRewards();
        updateRewardsDisplay();
        showNotification('تم إعادة تعيين نظام المكافآت للدردشة!', 'info');
    }
}

// حفظ نظام المكافآت
function saveChatRewards() {
    localStorage.setItem('chatRewards', JSON.stringify(appData.chatRewards));
    
    // حفظ في Firebase إذا كان هناك مستخدم
    if (appData.currentUser) {
        database.ref(`chatRewards/${appData.currentUser.id}`).set(appData.chatRewards);
    }
}

// إضافة رسالة لنظام المكافآت
function addMessageToRewards() {
    const currentStage = appData.chatRewards.currentStage;
    const stage = appData.chatRewards.stages[currentStage - 1];
    
    appData.chatRewards.messagesInCurrentStage++;
    
    // التحقق إذا اكتملت المرحلة
    if (appData.chatRewards.messagesInCurrentStage >= stage.messages) {
        // منح المكافأة
        if (appData.currentUser) {
            appData.currentUser.points += stage.reward;
            database.ref('users/' + appData.currentUser.id + '/points').transaction(current => {
                return (current || 0) + stage.reward;
            });
            
            showNotification(`🎉 مبروك! حصلت على ${stage.reward} نقطة لإكمالك المرحلة ${currentStage}!`, 'success');
        }
        
        // الانتقال للمرحلة التالية أو إعادة من البداية
        if (currentStage < appData.chatRewards.stages.length) {
            appData.chatRewards.currentStage++;
            appData.chatRewards.messagesInCurrentStage = 0;
        } else {
            // إكمال جميع المراحل، العودة للمرحلة الأولى
            appData.chatRewards.currentStage = 1;
            appData.chatRewards.messagesInCurrentStage = 0;
        }
    }
...
function setupEventListeners() {
    ...
    if (elements.editBioBtn) {
        elements.editBioBtn.addEventListener('click', showEditBioModal);
    }
    
    if (elements.newBioInput) {
        elements.newBioInput.addEventListener('input', (e) => {
            elements.bioCharCount.textContent = e.target.value.length;
        });
    }
    ...
    // مستمع سبب الميوت المخصص
    if (elements.muteReasonSelect) {
        elements.muteReasonSelect.addEventListener('change', (e) => {
            if (e.target.value === 'other') {
                elements.customMuteReason.style.display = 'block';
            } else {
                elements.customMuteReason.style.display = 'none';
            }
        });
    }
    
    // تأكيد الميوت
    if (elements.confirmMuteBtn) {
        elements.confirmMuteBtn.addEventListener('click', confirmMute);
    }
...
function updateUserInfo() {
    if (appData.currentUser) {
        ...
        // البروفايل
        if (elements.bioContent) {
            elements.bioContent.textContent = appData.currentUser.bio || 'لا يوجد وصف شخصي';
        }
        ...
        // البروفايل
        if (elements.bioContent) {
            elements.bioContent.textContent = 'غير متاح للزوار';
        }
    }
}
...
function loadMembers() {
    ...
    if (searchValue) {
        filteredMembers = filteredMembers.filter(m => 
            ...
            m.bio?.toLowerCase().includes(searchValue)
        );
    }
...
function createMemberCard(member) {
    ...
    // لون الاسم
    const nameColorClass = member.nameColor ? `name-color-${member.nameColor}` : '';
    ...
    card.innerHTML = `
        ...
        <div class="member-info">
            <div class="member-name ${nameColorClass}">
                <span>${member.name}</span>
                ${activeLogo ? `<span class="member-emblem">${activeLogo}</span>` : ''}
            </div>
...
// تحديث رسائل الدردشة مع الرتب وألوان الأسماء
function updateChatMessages() {
    ...
    appData.chatMessages.forEach(msg => {
        ...
        // لون الاسم
        const nameColorClass = sender?.nameColor ? `name-color-${sender.nameColor}` : '';
        
        messageElement.innerHTML = `
            <img src="${sender ? sender.image : 'https://files.catbox.moe/7pcx7j.jpg'}" class="avatar" alt="${msg.senderName}">
            <div class="chat-message-content">
                <div class="chat-message-header">
                    <span class="chat-message-sender ${nameColorClass}">
                        ${msg.senderName}
...
function checkForSpam(message) {
    ...
    appData.spamDetection.lastMessages.push({
        userId: userId,
        message: message,
        timestamp: now
    });
    ...
    appData.spamDetection.lastMessages = appData.spamDetection.lastMessages.filter(
        msg => now - msg.timestamp < 10000
    );
    
    // حساب تكرار الرسائل المتطابقة
    const userMessages = appData.spamDetection.lastMessages.filter(msg => msg.userId === userId);
    const identicalMessages = userMessages.filter(msg => msg.message === message).length;
    
    // إذا تجاوز العتبة، تطبيق الميوت
    if (identicalMessages >= appData.spamDetection.spamThreshold) {
        const muteUntil = now + appData.spamDetection.muteDuration;
        appData.chatMutedUntil = muteUntil;
        
        showNotification('تم كشف سبام متكرر! تم ميوتك لمدة 5 دقائق.', 'error');
        return true;
    }
...
function sendMessage() {
    ...
    const newMessage = {
        ...
        senderNameColor: appData.currentUser.nameColor,
        text: message,
        timestamp: Date.now()
    };
...
function loadShopItems(category = 'ranks') {
    ...
    items.forEach(item => {
        ...
        if (item.image && item.image.startsWith('#')) {
            ...
            // معاينة لون الاسم
            iconHTML = `<div class="shop-item-icon" style="font-size: 3rem; color: ${item.color}; text-shadow: 0 0 10px ${item.color};">A</div>`;
        } else {
    ...
    showConfirmModal(
        ...
        () => {
            ...
            if (item) {
                ...
                if (itemType === 'name-color') newItem.cssClass = item.cssClass;
                ...
                .then(() => {
                    ...
                    // إذا كان لون اسم، تفعيله تلقائياً
                    if (itemType === 'name-color') {
                        appData.currentUser.nameColor = item.id.replace('name-color-', '');
                        database.ref('users/' + appData.currentUser.id + '/nameColor').set(appData.currentUser.nameColor);
                    }
...
// تحديث عناصر السوق مع تصحيح مشكلة الشراء
function updateMarketItems() {
    ...
    filteredItems.forEach(item => {
        ...
        marketItem.className = 'shop-item market-item';
        marketItem.dataset.itemId = item.id;
    ...
    document.querySelectorAll('.btn-buy-market').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            showBuyMarketItemModal(itemId);
        });
    });
}

// عرض نافذة شراء عنصر من السوق
function showBuyMarketItemModal(itemId) {
    ...
    // حفظ العنصر المحدد
    appData.selectedMarketItem = marketItem;
    
    // تعبئة معلومات العنصر في النافذة
    elements.buyItemName.textContent = marketItem.name;
    elements.buyItemDescription.textContent = marketItem.description || 'لا يوجد وصف';
    elements.buyItemPrice.textContent = marketItem.price.toLocaleString();
    elements.buyItemSellerAvatar.src = marketItem.sellerImage;
    elements.buyItemSellerName.textContent = marketItem.sellerName;
    
    // تعبئة معاينة العنصر
    let previewHTML = '';
    if (marketItem.image && marketItem.image.startsWith('#')) {
        previewHTML = `<div style="width: 100px; height: 100px; margin: 0 auto; background-color: ${marketItem.image}; border-radius: 10px; border: 3px solid white;"></div>`;
    } else if (marketItem.image) {
        previewHTML = `<img src="${marketItem.image}" alt="${marketItem.name}" style="max-width: 100px; max-height: 100px; border-radius: 10px;">`;
    } else if (marketItem.type === 'logo') {
        previewHTML = `<div style="font-size: 4rem; text-align: center;">${marketItem.logo || '🎁'}</div>`;
    } else if (marketItem.type === 'name-color') {
        previewHTML = `<div style="font-size: 4rem; color: ${marketItem.color}; text-shadow: 0 0 10px ${marketItem.color}; text-align: center;">A</div>`;
    } else {
        previewHTML = `<div style="font-size: 4rem; color: ${marketItem.color || '#B1121A'}; text-align: center;"><i class="fas fa-crown"></i></div>`;
    }
    
    elements.buyItemPreview.innerHTML = previewHTML;
    
    // عرض النافذة
    elements.buyModalWindow.classList.add('active');
}

// تأكيد شراء عنصر من السوق
function confirmBuy() {
    if (!appData.selectedMarketItem) {
        showNotification('لم يتم اختيار عنصر', 'error');
        return;
    }
    
    const marketItem = appData.selectedMarketItem;
    ...
    showConfirmModal(
        'تأكيد الشراء',
        `هل تريد شراء "${marketItem.name}" بسعر ${marketItem.price.toLocaleString()} نقطة؟`,
        () => {
            // خصم النقاط من المشتري
            appData.currentUser.points -= marketItem.price;
            
            // إضافة النقاط للبائع
            const seller = membersData.find(m => m.id === marketItem.sellerId);
            if (seller) {
            ...
            const newItem = {
                ...
                purchasedAt: Date.now(),
                fromMarket: true // تمييز العنصر كمشترى من السوق
            };
            
            if (marketItem.image) newItem.image = marketItem.image;
            if (marketItem.logo) newItem.logo = marketItem.logo;
            if (marketItem.color) newItem.color = marketItem.color;
            if (marketItem.description) newItem.description = marketItem.description;
            if (marketItem.cssClass) newItem.cssClass = marketItem.cssClass;
            
            appData.currentUser.items.push(newItem);
            
            // تحديث العنصر في السوق ليكون مباعاً
            marketItem.sold = true;
            marketItem.buyerId = appData.currentUser.id;
            marketItem.buyerName = appData.currentUser.name;
            marketItem.soldAt = Date.now();
            ...
            updates[`users/${appData.currentUser.id}/items`] = appData.currentUser.items;
            
            // تحديث نقاط البائع
            if (seller) {
                updates[`users/${marketItem.sellerId}/points`] = seller.points;
                
                // إزالة العنصر من ممتلكات البائع
                if (seller.items) {
                    const sellerItemIndex = seller.items.findIndex(item => 
                        item.id === marketItem.itemId && item.type === marketItem.type
                    );
                    
                    if (sellerItemIndex !== -1) {
                        seller.items.splice(sellerItemIndex, 1);
                        updates[`users/${marketItem.sellerId}/items`] = seller.items;
                    }
                }
            }
            
            // تحديث السوق
            updates[`marketItems/${marketItem.id}`] = marketItem;
            ...
            database.ref().update(updates)
                .then(() => {
                    ...
                    // إغلاق النافذة
                    elements.buyModalWindow.classList.remove('active');
                    appData.selectedMarketItem = null;
                    ...
                    // إعادة تعيين المتجر
                    if (document.querySelector('.shop-tab.active')) {
                        loadShopItems(document.querySelector('.shop-tab.active').dataset.tab);
                    }
                })
                .catch(error => {
...
function loadCurrencySection() {
    ...
    // تحميل سجل التحويلات
    loadTransferHistoryFromFirebase();
    
    // تحميل تاريخ الأكواد
    loadCodesHistoryFromFirebase();
}
...
function transferPoints() {
    ...
    showConfirmModal(
        ...
        () => {
            ...
            // حفظ في Firebase
            const transferId = database.ref('transfers').push().key;
            const transferData = {
                fromId: appData.currentUser.id,
                fromName: appData.currentUser.name,
                toId: toId,
            ...
            updates[`transfers/${transferId}`] = transferData;
...
// تحميل سجل التحويلات من Firebase
function loadTransferHistoryFromFirebase() {
    if (!elements.transferHistory) return;
    
    elements.transferHistory.innerHTML = '<div class="empty-message">جاري تحميل سجل التحويلات...</div>';
}
...
function updateTransferHistory(transfers) {
    ...
    transfers.forEach(transfer => {
        ...
        const date = new Date(transfer.timestamp).toLocaleString('ar-EG');
        const isOutgoing = transfer.fromId === appData.currentUser?.id;
        const memberName = isOutgoing ? transfer.toName : transfer.fromName;
        const prefix = isOutgoing ? 'إلى' : 'من';
        
        historyItem.innerHTML = `
            <div class="history-info">
                <div class="history-title">تحويل ${prefix} ${memberName}</div>
                <div class="history-date">${date}</div>
            </div>
            <div class="history-amount ${isOutgoing ? 'negative' : 'positive'}">
                ${isOutgoing ? '-' : '+'}${transfer.amount.toLocaleString()}
            </div>
        `;
        
        elements.transferHistory.appendChild(historyItem);
    });
}

// إضافة تحويل للسجل
function addTransferToHistory(transferData) {
    if (!elements.transferHistory) return;
    
    const historyItem = document.createElement('div');
    historyItem.className = 'history-item';
    
    const date = new Date(transferData.timestamp).toLocaleString('ar-EG');
    const isOutgoing = transferData.fromId === appData.currentUser?.id;
    
    historyItem.innerHTML = `
        <div class="history-info">
            <div class="history-title">تحويل ${isOutgoing ? 'إلى' : 'من'} ${isOutgoing ? transferData.toName : transferData.fromName}</div>
            <div class="history-date">${date}</div>
        </div>
        <div class="history-amount ${isOutgoing ? 'negative' : 'positive'}">
            ${isOutgoing ? '-' : '+'}${transferData.amount.toLocaleString()}
        </div>
    `;
        ...
        .then(snapshot => {
            ...
            database.ref().update(updates)
                .then(() => {
                    ...
                    if (elements.giftCodeInput) elements.giftCodeInput.value = '';
...
// تحميل تاريخ الأكواد من Firebase
function loadCodesHistoryFromFirebase() {
    if (!elements.codesHistory) return;
    
    if (!appData.currentUser) {
        elements.codesHistory.innerHTML = '<div class="empty-message">يجب تسجيل الدخول</div>';
        return;
    }
    
    database.ref('codeHistory/' + appData.currentUser.id).limitToLast(20).once('value')
        .then(snapshot => {
            const codes = [];
            if (snapshot.exists()) {
                const data = snapshot.val();
            ...
            updateCodesHistory(codes);
        });
}

// تحديث تاريخ الأكواد
function updateCodesHistory(codes) {
    if (!elements.codesHistory) return;
    ...
    elements.codesHistory.innerHTML = '';
    
    codes.forEach(code => {
        const historyItem = document.createElement('div');
        historyItem.className = 'code-history-item';
        
        const date = new Date(code.timestamp).toLocaleString('ar-EG');
        
        historyItem.innerHTML = `
            <div class="code-history-info">
                <div class="code-history-title">كود: ${code.code}</div>
                <div class="code-history-date">${date}</div>
            </div>
            <div class="history-amount positive">+${code.value.toLocaleString()}</div>
        `;
        
        elements.codesHistory.appendChild(historyItem);
    });
}
...
function addCodeToHistory(value, code) {
    ...
    const codeHistoryId = database.ref('codeHistory/' + appData.currentUser.id).push().key;
    ...
    database.ref('codeHistory/' + appData.currentUser.id + '/' + codeHistoryId).set(codeHistoryData);
}
...
function loadOwnedItems() {
    ...
    for (const type in categorizedItems) {
        if (categorizedItems[type].length > 0) {
            ...
            typeTitle.textContent = getTypeName(type);
...
function checkIfItemIsActive(item) {
    ...
    if (item.type === 'background') {
        ...
        return appData.currentUser.nameColor === item.id.replace('name-color-', '');
    }
...
function useItem(itemId, itemType) {
    ...
    if (itemType === 'rank') updateData.rank = itemId;
    if (itemType === 'name-color') updateData.nameColor = appData.currentUser.nameColor;
    
    database.ref('users/' + appData.currentUser.id).update(updateData)
    .then(() => {
...
function showEditBioModal() {
    ...
    // تعبئة الحقل بالبايو الحالي
    elements.newBioInput.value = appData.currentUser.bio || '';
    elements.bioCharCount.textContent = elements.newBioInput.value.length;
    
    elements.editBioModal.classList.add('active');
}
...
function saveBio() {
    ...
    if (newBio.length > 200) {
        showNotification('الوصف طويل جداً (الحد الأقصى 200 حرف)', 'error');
        return;
    }
    
    if (appData.currentUser.points < bioCost) {
        showNotification(`نقاطك غير كافية. تحتاج إلى ${bioCost} نقطة`, 'error');
        return;
    }
    
    showConfirmModal(
        'تعديل الوصف الشخصي',
        `هل تريد تغيير وصفك الشخصي؟ سيكلفك ${bioCost} نقطة.`,
        () => {
            // خصم النقاط
            appData.currentUser.points -= bioCost;
            ...
            .then(() => {
                showNotification('تم تحديث وصفك الشخصي بنجاح', 'success');
                ...
                // إغلاق النافذة
                elements.editBioModal.classList.remove('active');
            })
            .catch(error => {
                console.error('Error updating bio:', error);
                showNotification('حدث خطأ في تحديث الوصف', 'error');
            });
        }
    );
}
...
function loadAdminPanel() {
    elements.adminContent.innerHTML = `
            ...
            <div class="admin-form">
                <div class="admin-form-group">
                    ...
                    <select id="newItemCategory">
                        ...
                        <option value="name-colors">ألوان الأسماء</option>
        ...
        <div class="admin-tab-content" id="violations-listTab">
            <h3>قائمة المخالفات</h3>
            <div id="violationsList" class="violations-list">
                <div class="empty-message">جاري تحميل المخالفات...</div>
            </div>
        </div>
...
function setupAdminEventListeners() {
    ...
    document.getElementById('addItemBtn')?.addEventListener('click', () => {
        ...
        if (category === 'ranks' || category === 'logos' || category === 'name-colors') newItem.color = color;
        if (category === 'name-colors') newItem.cssClass = `name-color-${name.toLowerCase().replace(/\s+/g, '-')}`;
    ...
    document.getElementById('muteMemberBtn')?.addEventListener('click', () => {
        ...
        if (!memberId) {
            showNotification('يجب اختيار عضو', 'error');
            return;
        }
        ...
        elements.muteModal.classList.add('active');
    });
        ...
        showConfirmModal(
            ...
            () => {
                ...
                if (period === 'all') {
                    ...
                    database.ref('chatMessages').orderByChild('timestamp').endAt(cutoffTime).once('value')
                        .then(snapshot => {
                            const updates = {};
                            snapshot.forEach(child => {
                                updates[`chatMessages/${child.key}`] = null;
                            });
                            database.ref().update(updates)
                                .then(() => {
                                    showNotification(`تم حذف الرسائل القديمة`, 'success');
                                });
                        })
                        .catch(error => {
...
function confirmMute() {
    const memberId = appData.selectedMember;
    const reason = elements.muteReasonSelect.value === 'other' 
        ? elements.customMuteReason.value.trim()
        : elements.muteReasonSelect.value;
    const duration = parseInt(elements.muteDurationSelect.value) * 60000; // تحويل لدقائق
    
    if (!reason) {
        showNotification('يجب تحديد سبب الميوت', 'error');
        return;
    }
    ...
    const violationId = database.ref('violations').push().key;
    const violationData = {
        memberId: memberId,
        memberName: member.name,
        reason: reason,
        duration: duration / 60000, // بالدقائق
        mutedBy: appData.currentUser.id,
        mutedByName: appData.currentUser.name,
        mutedAt: Date.now(),
        muteUntil: muteUntil
    };
    
    database.ref('violations/' + violationId).set(violationData)
        .then(() => {
            showNotification(`تم ميوت ${member.name} لمدة ${duration/60000} دقيقة`, 'success');
            elements.muteModal.classList.remove('active');
            
            // إعادة تعيين الحقول
            elements.muteReasonSelect.value = 'spam';
            elements.customMuteReason.value = '';
            elements.customMuteReason.style.display = 'none';
            elements.muteDurationSelect.value = '5';
        ...
        .catch(error => {
            ...
            showNotification('حدث خطأ في تطبيق الميوت', 'error');
        });
}
...
// تحميل قائمة المخالفات
function loadViolationsList() {
    const violationsList = document.getElementById('violationsList');
    if (!violationsList) return;
    
    database.ref('violations').orderByChild('mutedAt').limitToLast(50).once('value')
        .then(snapshot => {
            const violations = [];
            if (snapshot.exists()) {
                const data = snapshot.val();
            ...
            if (violations.length === 0) {
                violationsList.innerHTML = '<div class="empty-message">لا توجد مخالفات</div>';
                return;
            }
            
            // ترتيب من الأحدث إلى الأقدم
            violations.sort((a, b) => b.mutedAt - a.mutedAt);
            ...
            violations.forEach(violation => {
                ...
                const date = new Date(violation.mutedAt).toLocaleString('ar-EG');
                const muteUntil = new Date(violation.muteUntil).toLocaleString('ar-EG');
                const now = Date.now();
                const isActive = now < violation.muteUntil;
                
                violationElement.innerHTML = `
                    <div class="violation-info">
                        <h4>${violation.memberName}</h4>
                        <p><strong>السبب:</strong> ${violation.reason}</p>
                        <p><strong>المدة:</strong> ${violation.duration} دقيقة</p>
                        <p><strong>الميوت حتى:</strong> ${muteUntil}</p>
                        <p><strong>بواسطة:</strong> ${violation.mutedByName}</p>
                        <p><strong>التاريخ:</strong> ${date}</p>
                    </div>
                    <div class="violation-status ${isActive ? 'active' : 'expired'}">
                        ${isActive ? 'نشط' : 'منتهي'}
                    </div>
                `;
        ...
        .catch(error => {
            ...
            violationsList.innerHTML = '<div class="empty-message">حدث خطأ في تحميل المخالفات</div>';
        });
}
...
function submitRating() {
    ...
    const member = membersData.find(m => m.id === appData.selectedMember);
    if (!member) {
        showNotification('العضو غير موجود', 'error');
        return;
    }
    
    if (member.id === appData.currentUser.id) {
    ...
    // التحقق إذا كان المستخدم قد قيم هذا العضو من قبل
    const existingReviewIndex = member.reviews.findIndex(review => review.userId === appData.currentUser.id);
    ...
    if (existingReviewIndex !== -1) {
        member.reviews[existingReviewIndex] = review;
    } else {
        member.reviews.push(review);
    }
    
    // حساب متوسط التقييم
    const totalRating = member.reviews.reduce((sum, review) => sum + review.rating, 0);
    member.rating = totalRating / member.reviews.length;
    ...
    .then(() => {
        showNotification(`تم تقييم ${member.name} بنجاح`, 'success');
...
function confirmSell() {
    ...
    // التحقق من أن العنصر ليس من السوق منذ أقل من 24 ساعة
    if (item.fromMarket && Date.now() - item.purchasedAt < 86400000) {
        showNotification('لا يمكن بيع العناصر المشتراة من السوق خلال أول 24 ساعة', 'error');
        return;
    }
    ...
    if (item.color) newMarketItem.color = item.color;
    if (item.description) newMarketItem.description = item.description;
    if (item.cssClass) newMarketItem.cssClass = item.cssClass;
...
// معالجة تسجيل الخروج مع تحديث حالة الاتصال
...
function showBannedScreen(reason) {
    elements.banReasonText.textContent = reason;
